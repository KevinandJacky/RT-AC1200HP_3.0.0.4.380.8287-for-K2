...
#if defined(CONFIG_RA_HW_NAT) || defined(CONFIG_RA_HW_NAT_MODULE)
static inline int local_proto(u_int8_t protonum, unsigned int dataoff, struct sk_buff *skb)
{
		...
		struct sk_buff *skb)
{
	...
	help = nfct_help(ct);
	if ((help && help->helper) ||
	    (hooknum == NF_INET_LOCAL_OUT) || local_proto(protonum, dataoff, skb)) {
