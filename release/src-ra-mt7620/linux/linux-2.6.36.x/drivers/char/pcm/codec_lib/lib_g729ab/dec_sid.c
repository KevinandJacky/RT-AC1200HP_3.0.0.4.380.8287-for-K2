...
void Init_Dec_cng(void)
{
  ...
  pg729dec->pdec_sid->sid_gain = tab_Sidgain[0];
...
)
{
  ...
  Word16 dif;
	...
	dec_sid_type* 		pdec_sid = pg729dec->pdec_sid;
  ...
  if(parm[0] != 0) {
    ...
    pdec_sid->sid_gain = tab_Sidgain[(int)parm[4]];           
    
    /* Inverse quantization of the LSP */
    sid_lsfq_decode(&parm[1], pdec_sid->lspSid, freq_prev);
  ...
  /*****************/
    ...
    if(dif == 0) {
      ...
      pdec_sid->sid_gain = tab_Sidgain[(int)ind];
    }
    
  }
  
  if(dif == 0) {
    pdec_sid->cur_gain = pdec_sid->sid_gain;
  }
  else {
    pdec_sid->cur_gain = mult_r(pdec_sid->cur_gain, A_GAIN0);
    pdec_sid->cur_gain = add(pdec_sid->cur_gain, mult_r(pdec_sid->sid_gain, A_GAIN1));
  }
 
  Calc_exc_rand(pdec_sid->cur_gain, exc, seed, FLAG_DEC);

  /* Interpolate the Lsp vectors */
  Int_qlpc(lsp_old, pdec_sid->lspSid, A_t);
  Copy(pdec_sid->lspSid, lsp_old, M);
...
void Init_lsfq_noise_enc(void)
{
  ...
  for (i=0; i<4; i++)
    Copy(fg[0][i], pg729enc->pqsidlsf->noise_fg[0][i], M);
  
  for (i=0; i<4; i++)
    for (j=0; j<M; j++){
      acc0 = L_mult(fg[0][i][j], 19660);
      acc0 = L_mac(acc0, fg[1][i][j], 13107);
      pg729enc->pqsidlsf->noise_fg[1][i][j] = extract_h(acc0);
    }
}
...
void Init_lsfq_noise_dec(void)
{
  ...
  /* initialize the noise_fg */
  for (i=0; i<4; i++)
    Copy(fg[0][i], pg729dec->pdec_sid->noise_fg[0][i], M);
  
  for (i=0; i<4; i++)
    for (j=0; j<M; j++){
      acc0 = L_mult(fg[0][i][j], 19660);
      acc0 = L_mac(acc0, fg[1][i][j], 13107);
      pg729dec->pdec_sid->noise_fg[1][i][j] = extract_h(acc0);
    }
}
                     ...
                     )
{
  ...
  Lsp_prev_compose(tmpbuf, lsfq, pg729dec->pdec_sid->noise_fg[index[0]], freq_prev, 
